...
> 定位：字节、Unicode、字符串构造与转换。
> 前置知识：变量、控制流与基本 Go 语法。
> 配套示例：[str/main.go](str/main.go)（`go run ./str`）；命令均在 notes 根目录执行。

**阅读路线**：先读 [基础](#topic-1) → [常见陷阱](#topic-3)；深入实现或进阶用法时读 [内存表示、转换与零拷贝](#topic-2)。
...
代价：**任何修改都要重新分配**。循环里 `s += x` 是 O(n²)（见 [拼接的五种写法](#section-3-5)）。

<a id="section-1-3"></a>

### 1.2 string / []byte / []rune
...
| 转换 | 成本 |
| --- | --- |
| `[]byte(s)` / `string(b)` | 分配 + `memmove`（除了 [编译器的零拷贝特例](#section-2-1) 的编译器特例） |
...
1. **`String()` 是零拷贝的**（内部 `unsafe.String(unsafe.SliceData(buf), len(buf))`）；
2. `String()` 之后的 `Write` 只在已有内容之后追加，不改动已返回字符串的字节范围；容量不足才扩容复制，`Reset` 则丢弃原缓冲区；
...
`fmt.Sprintf` 在热路径上是常见的性能杀手：参数要装箱（[常见的逃逸原因](mem.md#section-2-2)）、格式串要解析、内部还有 `sync.Pool`。能用 `strconv` 就别用 `fmt`。
...
**什么时候值得用**：热路径上 KB 级以上的转换，典型是 HTTP body / 文件内容的解析——注意此时 `[]byte` 通常来自 `sync.Pool`，一旦回池字符串就悬空了（[Put 之后继续使用对象](pool.md#section-4-3)）。
...
`sync.Map` 在 1.24 换成 hash-trie，最初的动机就是给 `unique` 包用（见 [sync.Map：1.24 之后是 concurrent hash-trie](sync.md#section-2-9)）。
...
和 slice 的同一个坑（[大 slice 截取小 slice 造成的内存泄漏](slice.md#section-3-3)）。**长期保存的子串一律 `strings.Clone`**。典型事故：解析大 JSON / 日志行，只留几个字段却让整个 buffer 泄漏。

<a id="section-3-7"></a>

### 3.5 `utf8.RuneCountInString` vs `len([]rune(s))`
...
`stringStruct{str unsafe.Pointer; len int}`，16 字节。没有 cap 是因为不可变——不需要预留增长空间。这也让 `s[i:j]` 天然零拷贝（见 [内存布局](#section-1-1)）。

**2. 字符串为什么设计成不可变？**
换来三件事：切片/传参零拷贝、可做 map key（哈希稳定）、并发读免锁。代价是任何修改都要重新分配，循环拼接是 O(n²)（见 [不可变意味着什么](#section-1-2)、[拼接的五种写法](#section-3-5)）。

**3. `len(s)` 返回什么？怎么数字符数？**
字节数。字符数用 `utf8.RuneCountInString(s)`。注意"字符"还有第三层——grapheme cluster（`"é"` 是 2 个 rune 但看起来 1 个字符），要靠 `x/text`（见 [UTF-8 要点](#section-1-5)、[`len(s)` 不是字符数](#section-3-2)）。

**4. `string` 和 `[]byte` 互转要拷贝吗？有哪些例外？**
一般都要拷贝。编译器有三条零拷贝特例（`runtime/string.go:194` 注释）：`m[string(b)]`、`"<"+string(b)+">"`、`string(b)=="literal"`。**`for range string(b)` 不在其中**，实测会拷贝（见 [编译器的零拷贝特例](#section-2-1)）。

**5. 怎么零拷贝地在 string 和 []byte 之间转换？有什么风险？**
1.20 起用 `unsafe.String(unsafe.SliceData(b), len(b))` 和 `unsafe.Slice(unsafe.StringData(s), len(s))`。风险：转换后修改 `[]byte` 会让"不可变"的字符串变化；`string → []byte` 方向写入字面量会 SIGSEGV（不可 recover）。实测能省 1650 倍（12KB：1535ns → 0.93ns），只在热路径上大块数据时值得（见 [`unsafe` 零拷贝转换（1.20+ 的正确写法）](#section-2-2)）。

**6. `for i, r := range s` 里 i 和 r 分别是什么？**
`i` 是字节下标（不连续），`r` 是解码出的 rune。非法 UTF-8 得到 `U+FFFD` 且 i 只前进 1（见 [`range` 的语义](#section-1-4)）。

**7. `s[0]` 为什么打印出来是数字/乱码？**
`s[i]` 的类型是 `byte`（`uint8`），不是字符。对多字节字符取单个字节没有意义。正确做法是 `utf8.DecodeRuneInString` 或 `[]rune(s)[0]`（见 [`s[i]` 是字节，不是字符](#section-3-1)）。

**8. 拼接字符串有哪些方式？分别什么时候用？**
固定少量 → `a+b+c`（一次 `concatstrings`）；循环 → `strings.Builder`+`Grow`；已有 slice → `strings.Join`；需要 append 语义/复用 buffer → `[]byte`+`strconv.AppendXxx`。实测 `Builder+Grow` 比循环 `+=` 快 9 倍、分配少 99 倍（见 [拼接的五种写法](#section-3-5)）。

**9. `strings.Builder` 和 `bytes.Buffer` 有什么区别？**
`Builder` 只写不读，`String()` 零拷贝（`unsafe.String` 指向内部 buf），不能拷贝（有 `addr` 自引用检查）。`Buffer` 支持读写，`String()` 会拷贝。只拼字符串用前者（见 [`strings.Builder`](#section-1-6)）。

**10. 为什么子串会造成内存泄漏？**
`s[i:j]` 共享底层数组，一个 10 字节的子串能让 10MB 的原字符串无法回收。长期保存的子串要 `strings.Clone`（见 [子串持有整个底层数组](#section-3-4)）。

**11. 忽略大小写比较字符串的正确方式？**
`strings.EqualFold`（零分配，做 Unicode case folding）。`ToLower(a)==ToLower(b)` 有两次分配且慢 5 倍。安全敏感的比较用 `crypto/subtle.ConstantTimeCompare`（见 [大小写与比较](#section-3-3)）。

**12. `string(65)` 得到什么？**
`"A"`（把 65 当 rune）。Go 1.15 起 `go vet` 会报错提示。要 `"65"` 用 `strconv.Itoa`（见 [数字转换](#section-3-6)）。

**13. `strconv.Itoa` 和 `fmt.Sprintf("%d", n)` 差多少？**
实测 24.3ns vs 66.7ns（2.7 倍）。`fmt` 要装箱参数、解析格式串、走反射。零分配版本是 `strconv.AppendInt(buf[:0], n, 10)`（12.7ns，0 allocs）（见 [数字转换](#section-3-6)）。

**14. `[]rune(s)` 的开销是多少？**
分配 `4 × RuneCount` 字节 + 逐个解码。实测 12KB 字符串转 `[]rune` 是转 `[]byte` 的 12 倍时间、4 倍内存。只在真的需要随机访问字符时才转（见 [string / []byte / []rune](#section-1-3)）。

**15. 什么是字符串驻留？Go 里怎么做？**
把内容相同的字符串收敛到同一份内存，减少重复占用。1.23 起用 `unique.Make[T]` —— 返回可比较的 8 字节 `Handle`，无引用时会被 GC 自动清理（weak pointer）。自己写 map 池的问题是永不释放且要加锁（见 [字符串驻留（interning）](#section-2-3)）。
