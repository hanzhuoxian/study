...
> 定位：接口契约、类型断言与动态派发。
> 前置知识：[方法](method.md)。
> 配套示例：[interface/main.go](interface/main.go)（`go run ./interface`）；命令均在 notes 根目录执行。
...
- 接口变量在运行时持有**动态类型 + 动态值**两部分信息（详见 [接口值的两种运行时表示：`eface` 与 `iface`](#section-2-1)），调用 `w.Write(...)` 时按动态类型分派到具体实现。
...
- Go 1.18 起优先用 `any` 书写，语义与 `interface{}` 完全相同，只是可读性更好；很多场景下**应优先考虑泛型**而不是空接口（见 [用空接口代替泛型，丢失类型安全](#section-3-5)）。
...
- `x.(T)` 用于把接口值还原成具体类型或另一个接口；两个返回值的形式是工程中的默认写法（见 [类型断言用了不安全形式，断言失败直接 panic](#section-3-3)）。

**本质**：接口值在运行时是 `(动态类型, 动态值)` 二元组（见 [接口值的两种运行时表示：`eface` 与 `iface`](#section-2-1)），类型断言就是对"动态类型"这一栏做运行时检查，检查通过后按 `T` **的方式重新解释这个值**。运行时检查动态类型，但编译器仍会根据静态类型检查断言是否可能成立。`x` 必须是接口类型，对具体类型断言是编译错误：
...
| 接口类型   | 动态类型是否实现 T 的方法集 | 仍是接口值，动态类型/值不变，只换"壳"（方法集变化） | 查 itab 表，见 [类型断言的运行时实现与开销](#section-2-6) |
...
**断言成功 ≠ 拿到非 nil 值**：断言只保证类型对，不保证值非 nil，配合类型化 nil 陷阱（见 ["nil 接口" 陷阱：类型化的 nil 不等于接口 nil](#section-3-1)）：
...
泛型类型参数的断言限制和转换方式统一见 [泛型中的类型断言](generic.md#section-3-4)。
...
- `switch v := x.(type)` 是多路类型断言的语法糖，每个 `case` 分支里 `v` 的静态类型就是该分支匹配到的类型；`case nil` 专门匹配接口值为 `nil` 的情况（见 [nil 接口值 vs "接口里装了 nil"](#section-2-5)）。
...
- **不要滥用**：如果在写一长串 `type switch` 做分发，先考虑能不能把行为定义成接口方法让各类型自己实现（多态代替分发），或者用泛型（见 [用空接口代替泛型，丢失类型安全](#section-3-5)）。`type switch` 真正合理的场景是处理**外部输入的不确定类型**——JSON 解码结果、reflect、AST 遍历、序列化库。
...
- 若原值本来就是指针（如 `*T`），装箱几乎零开销，`Data` 直接存该指针；若是较大的值类型（如大 `struct`），通常需要在堆上分配一份拷贝，带来额外的内存分配和 GC 压力（见 [用空接口代替泛型，丢失类型安全](#section-3-5)）。
...
- 两个接口值相等，当且仅当**动态类型相同**且**动态值相等**（用 `==` 递归比较，规则与对应具体类型的 `==` 一致，见 [结构体的可比较性](struct.md#section-2-4)）。
- 若动态类型是不可比较类型（slice、map、function），比较时运行时直接 `panic: comparing uncomparable type`（见 [不可比较的动态类型放进接口比较会 panic](#section-3-2)）。
...
- 如果把一个类型化的 `nil` 指针（如 `var p *T = nil`）赋给接口变量，`Type` 会被设成 `*T` 的类型信息（非 nil），只有 `Data` 是 nil，此时接口变量本身 **`!= nil`**——这是 Go 里最著名的陷阱之一（见 ["nil 接口" 陷阱：类型化的 nil 不等于接口 nil](#section-3-1)）。
...
另外，具体类型断言拿到的是动态值的**拷贝**，大结构体会有一次拷贝开销（见 [接口赋值时的"装箱"与内存分配](#section-2-3)）。
...
- 根源见 [nil 接口值 vs "接口里装了 nil"](#section-2-5)：`err` 是 `*MyError` 类型的 `nil`，赋给 `error` 接口后 `Type = *MyError`、`Data = nil`，接口整体不等于 `nil`。
...
- 接口类型本身声明为"可比较"是**静态编译期**放行的（`interface{}` 支持 `==` 语法），但**运行时**才真正检查动态类型是否可比较（见 [接口值的比较](#section-2-4)），装的是 slice/map/function 时必然 panic。
...
- 除非能**确定**断言一定成功（如刚做过 `type switch` 分支判断），否则一律用 `v, ok := x.(T)` 的双返回值形式，`ok == false` 时优雅处理而不是让程序崩溃（见 [类型断言（type assertion）](#section-1-4)）。
...
- 见 [接口赋值时的"装箱"与内存分配](#section-2-3)：把非指针的大值类型反复装箱进空接口传参（常见于早期日志库、`fmt.Sprintf` 类可变参数函数），会产生大量堆分配，是热路径上容易被忽视的性能问题。
...
- `interface{}` 参数把类型检查从编译期推迟到运行时，需要大量类型断言/`type switch` 兜底，调用方传错类型编译器也不会报错；Go 1.18 引入泛型后，这类"要处理多种具体类型但逻辑一致"的场景应优先用泛型，只有真正需要"运行时不确定类型、需要动态判断"时才用接口（如 [类型选择（type switch）](#section-1-5) 的 `type switch` 场景、插件式架构）。
...
**装进接口的是值还是指针，直接决定方法集，进而决定接口断言成不成功**（方法集规则见 [方法集（method set）规则](method.md#section-2-2)、[值类型无法满足指针接收者方法的接口](method.md#section-3-2)）。排查"明明实现了接口却断言失败"时，先看是不是把值而非指针放进了接口。
...
Go 用结构化类型：类型不需要声明"我实现了某接口"，只要方法集包含接口要求的全部方法就自动满足，接口和实现可以分处不同、互不知情的包中；Java/C++ 是名义类型（nominal typing），必须显式 `implements`/继承。这让 Go 里可以先定义一个小接口，再让已有类型"事后满足"它，无需修改已有代码（见 [接口声明与隐式实现](#section-1-1)）。

**2. 接口值在运行时是什么结构？空接口和非空接口一样吗？**
不一样。接口值本质是"类型信息 + 数据指针"的二元组，但空接口（`EmptyInterface`）第一个字直接是 `*Type`；非空接口（`NonEmptyInterface`）第一个字是 `*ITab`（内部再包一层 `Type`），因为非空接口还需要一张方法表支持动态派发（见 [接口值的两种运行时表示：`eface` 与 `iface`](#section-2-1)）。

**3. itab 是什么？什么时候构建、要不要每次都重新计算？**
`itab` 是 `(接口类型, 动态类型)` 这一对信息对应的方法表，把"调用接口的第 k 个方法"翻译成"具体类型上第 k 个方法的函数地址"。运行时用全局哈希表缓存所有构建过的 `itab`，同一对 `(接口, 类型)` 只在第一次转换时构建一次，之后都是查表复用，转换本身开销很小（见 [itab：接口类型 + 动态类型 → 方法表](#section-2-2)）。

**4. 为什么 `var err error = (*MyError)(nil)` 之后 `err != nil`？**
接口变量等于 `nil` 要求类型指针和数据指针都为 `nil`。把一个类型化的 `nil` 指针赋给接口后，接口的类型部分被设成了 `*MyError`（非 nil），只有数据部分是 `nil`，所以接口整体不等于 `nil`（见 [nil 接口值 vs "接口里装了 nil"](#section-2-5)、["nil 接口" 陷阱：类型化的 nil 不等于接口 nil](#section-3-1)）。这是函数返回值类型不应该用具体错误类型指针、而应该直接用 `error` 接口类型的原因之一。

**5. 两个接口值用 `==` 比较，规则是什么？什么情况下会 panic？**
先比较动态类型是否相同，不同直接 `false`；相同再递归比较动态值是否相等。如果动态类型是不可比较类型（slice、map、func），运行时会 panic（见 [接口值的比较](#section-2-4)、[不可比较的动态类型放进接口比较会 panic](#section-3-2)）。

**6. 类型断言 `x.(T)` 和类型选择 `switch x.(type)` 有什么关系？**
`type switch` 本质是多路类型断言的语法糖，编译器会把每个 `case` 展开为一次类型断言（内部实际是按 `itab`/`Type` 做比较，用 `Hash` 字段加速），只是把多次判断合并到一个结构里，同时能拿到每个分支里正确静态类型的变量 `v`（见 [类型断言（type assertion）](#section-1-4)、[类型选择（type switch）](#section-1-5)、[itab：接口类型 + 动态类型 → 方法表](#section-2-2)）。

**7. 把一个 `struct` 值赋给 `interface{}` 会发生什么？**
会触发"装箱"：运行时在堆上（或利用逃逸分析优化在栈上，视情况而定）分配一份该值的拷贝，接口的 `Data` 指针指向这份拷贝；如果原值本身已经是指针，则直接存指针，几乎零开销。小整数（0~255）有专门的静态缓存优化，不必每次分配（见 [接口赋值时的"装箱"与内存分配](#section-2-3)）。

**8. 为什么说"频繁把大结构体传给 `interface{}` 参数"是性能陷阱？怎么优化？**
每次传参都要重新装箱拷贝一份数据到堆上，产生额外分配和 GC 压力。优化手段：改传指针（`*T` 装箱几乎无开销）；或者用泛型替代 `interface{}` 参数，编译期单态化生成具体类型版本的函数，完全避免装箱（见 [接口赋值时的"装箱"与内存分配](#section-2-3)、[接口频繁转换带来的堆分配开销](#section-3-4)、[用空接口代替泛型，丢失类型安全](#section-3-5)）。

**9. 什么时候该用接口，什么时候该用泛型？**
需要"同一段逻辑处理多种已知具体类型、编译期就能确定"时优先用泛型（类型安全、无装箱开销）；需要"运行时才知道具体类型、按类型做不同分支处理，或者要对外暴露一组行为契约、隐藏具体实现（依赖倒置/插件式架构）"时用接口。两者不是互斥的，标准库里两者常常配合使用（见 [面向接口编程：用接口解耦调用方与实现](#section-1-7)、[用空接口代替泛型，丢失类型安全](#section-3-5)）。

**10. 接口嵌入和结构体嵌入是同一种机制吗？**
不是。结构体嵌入（[通过嵌入获得方法](method.md#section-1-5)、[结构体嵌入（组合）](struct.md#section-1-5)）是把字段/方法"提升"到外层类型，编译器生成转发方法；接口嵌入只是把被嵌入接口的方法签名**合并**进外层接口的方法集合，接口本身不含任何实现，没有"提升"和转发的概念，纯粹是方法列表在编译期的拼接（见 [接口的组合（interface embedding）](#section-1-6)）。

**11. `x.(T)` 中 T 是接口和 T 是具体类型，运行时行为有何不同？**
T 是具体类型时只比较动态类型是否**严格相等**，成功后取出动态值，结果脱离接口（拆箱），开销是一次指针比较；T 是接口类型时要检查动态类型是否满足 T 的方法集，走 `getitab` 查表/构建 itab，结果仍是接口值，动态类型和动态值都不变，只是可调用的方法集变了（换壳）（见 [类型断言（type assertion）](#section-1-4)、[类型断言的运行时实现与开销](#section-2-6)）。

**12. `v, ok := err.(*MyError)`，`ok` 为 true 时 `v` 一定不是 nil 吗？**
不一定。断言只校验动态类型，动态值完全可以是 nil 指针（类型化 nil）。所以断言出指针类型后仍需判空（见 [类型断言（type assertion）](#section-1-4)、["nil 接口" 陷阱：类型化的 nil 不等于接口 nil](#section-3-1)）。

**13. 为什么 `err.(*os.PathError)` 在实际项目里经常失败？**
因为错误被 `%w` 包装过，动态类型变成了 `*fmt.wrapError`。应该用 `errors.As` 沿 `Unwrap` 链逐层断言（见 [错误被 `%w` 包装后，直接断言失效](#section-3-9)）。

**14. `switch v := x.(type)` 中 `case A, B:` 时 `v` 是什么类型？**
是 `x` 的静态类型（通常就是那个接口），**不会**被收窄到 A 或 B。只有单类型 case 才收窄（见 [类型选择（type switch）](#section-1-5)）。

**15. `case nil` 匹配的是什么？和"接口里装了 nil 指针"有什么区别？**
`case nil` 匹配的是动态类型为 nil、即接口值整体为 nil 的情况；接口里装了 nil 指针时动态类型非 nil，不会匹配 `case nil`，而会走对应的具体类型分支（见 [类型选择（type switch）](#section-1-5)、[nil 接口值 vs "接口里装了 nil"](#section-2-5)、["nil 接口" 陷阱：类型化的 nil 不等于接口 nil](#section-3-1)）。

**16. 对一个 `nil` 接口做类型断言会怎样？为什么说"断言成更宽松的接口≈赋值"有例外？**
nil 接口的动态类型也是 nil，断言成任何类型都失败：comma-ok 形式返回 false，单值形式 panic。而直接赋值不会 panic，这正是二者唯一的差别（见 [类型断言（type assertion）](#section-1-4)）。

**17. 标准库里类型断言最典型的用途是什么？**
能力探测：基础接口保持最小，可选能力通过断言发现。如 `io.Copy` 断言 `WriterTo`/`ReaderFrom` 走快速路径，`http.ResponseWriter` 断言 `Flusher`/`Hijacker` 拿额外能力，`fmt` 依次断言 `Formatter`/`Stringer`/`error` 选择格式化路径（见 [断言的典型用法：接口能力探测](#section-1-8)）。
