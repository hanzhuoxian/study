...
> 定位：G/M/P 的角色、资源与关系。
> 前置知识：[Channel](chan.md)、[sync](sync.md)。
> 配套示例：[gmp/main.go](gmp/main.go)（`go run ./gmp`）；命令均在 notes 根目录执行。

**阅读路线**：先读 [三者关系小结](#topic-4) → [各要素的诞生顺序（从进程到 goroutine）](#topic-5) → [调度是如何运转的](#topic-9)；深入实现或进阶用法时读 [G — goroutine 结构体](#topic-1) → [M — machine（OS 线程）结构体](#topic-2) → [P — processor（逻辑处理器）结构体](#topic-3)。

**篇内导航**

- [三者关系小结](#topic-4)
- [G — goroutine 结构体](#topic-1)
- [M — machine（OS 线程）结构体](#topic-2)
- [P — processor（逻辑处理器）结构体](#topic-3)
- [各要素的诞生顺序（从进程到 goroutine）](#topic-5)
- [调度是如何运转的](#topic-9)

> 环境：Go 1.26.3；结构节选自 `$GOROOT/src/runtime/runtime2.go`，字段与大小依版本和架构而变。

- **G（Goroutine）**：Go 的用户态协程，代表一个待执行的任务单元。创建成本低（初始栈仅 2KB，可动态伸缩），由 Go 运行时调度，数量可达百万级。
- **M（Machine）**：操作系统线程（OS thread），是真正执行代码的实体。M 必须绑定一个 P 才能运行 G，线程数不直接受 `GOMAXPROCS` 限制；另有运行时线程上限。
- **P（Processor）**：逻辑处理器，代表执行 G 所需的调度上下文与资源（如本地可运行 G 队列、内存分配缓存）。P 的数量由 `GOMAXPROCS` 决定（默认值及容器适配见 [GOMAXPROCS](sched.md#section-1-1)），决定了同时并行执行 Go 代码的最大并发度。
...
- 调度查找还包含公平性、GC 和定时器等分支，完整顺序见 [`schedule()` → `findRunnable()` 的查找顺序](sched.md#section-2-1)。
...
启动时先建立 `m0/g0`，再由 `schedinit` 创建 P，最后创建并调度入口为 `runtime.main` 的主 goroutine。完整汇编与初始化时间线见 [程序启动](exec.md)。

运行期创建一个 goroutine 通常只需新建或复用 G 并入队；需要更多执行线程时才唤醒空闲 M 或创建新 M。P 的数量由 `GOMAXPROCS` 控制，并可随设置变化调整。查找任务、抢占和系统调用移交统一见 [调度器](sched.md)。

<a id="topic-9"></a>

## 调度是如何运转的

**每个 M 都有自己的 `g0`，不是进程只有一个调度栈。** 全局变量 `runtime.g0` 是启动线程 `m0` 的那一份；其他 M 各有自己的系统栈。

调度并非由 `m0` 统一指挥。各 M 在自己的系统栈上运行调度逻辑，从队列取出可运行的 G，再恢复该 G 的执行现场。用户任务结束后回到 runtime 清理；这解释了 G 的 `sched` 与 M 的 `g0/curg` 字段为什么分开。

本篇负责 G/M/P 的角色、结构和关系。下一步按问题选择：

- 想知道“谁先创建”：读 [程序启动](exec.md)。
- 想知道“下一个任务怎么选”：读 [调度器](sched.md)。
- 想知道“IO 等待怎么恢复”：读 [netpoll](netpoll.md)。
- 想观察这些行为：在 notes 根目录运行 `go run ./gmp`，再用 `go tool trace ./tmp/trace.out` 查看运行期事件。
