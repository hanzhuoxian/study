...
> 定位：指针转换、适用边界与运行时检查。
> 前置知识：[内存分配与栈](mem.md)、[reflect](reflect.md)。
> 配套示例：[uns/main.go](uns/main.go)（`go run ./uns`）；命令均在 notes 根目录执行。

**阅读路线**：先读 [什么时候真的该用](#topic-4) → [基础](#topic-1) → [常见陷阱](#topic-3)；深入实现或进阶用法时读 [六种合法模式](#topic-2)。
...
- 热路径上 KB 级以上的 `[]byte` ↔ `string` 零拷贝（[`unsafe` 零拷贝转换（1.20+ 的正确写法）](string.md#section-2-2) 实测 1650 倍）；
...
字段顺序对大小的影响（[内存布局与字段对齐](struct.md#section-2-1) 的实测版）：
...
### 2.2 `unsafe.Pointer` 与 `uintptr` 的根本区别
...
所以 **`uintptr` 只能作为"某个表达式内部的临时算术中间值"存在**。把它存进变量、字段、map，就等于持有一个随时可能失效的地址（见 [把 `uintptr` 存进变量：最经典的 unsafe bug](#section-3-1)）。

`go vet` 会对可疑的 `unsafe.Pointer(u)` 报 `possible misuse of unsafe.Pointer`——而它报得对，我在示例里想演示这个反例都被它拦下了。

<a id="section-1-3"></a>

### 2.3 1.17/1.20 的新 API
...
**1.20 之后 `reflect.SliceHeader`/`StringHeader` 已 deprecated，不要再用**（见 [别自己拼 header](#section-3-4)）。
...
| 官方模式 | 本篇入口 |
| --- | --- |
| ① 类型重新解释 | [布局与大小前提](#section-2-1) |
| ② 指针转整数 | [其他模式](#section-2-5) |
| ③ 指针算术 | [同一表达式转换](#section-2-2) |
| ④ syscall 参数 | [调用期间保活](#section-2-3) |
| ⑤ reflect 返回 uintptr | [立即转换](#section-2-4) |
| ⑥ header 的 Data 字段 | [历史模式与替代 API](#section-2-5) |

`unsafe.Pointer` 的文档列出了**六种**合法模式，"不属于这些模式的代码今天很可能就是错的，或者将来会变错"。上表给出六种模式的入口，下面展开常用模式。

<a id="section-2-1"></a>

### 3.1 模式 ①：`*T1` → `Pointer` → `*T2`（类型重新解释）
...
### 3.2 模式 ③：`Pointer` → `uintptr` 做算术 → **立刻**转回 `Pointer`
...
### 3.4 模式 ⑤：`reflect.Value.UnsafeAddr` / `Pointer` 必须立刻转换
...
配合模式 ⑤ 可以读写未导出字段（[未导出字段](reflect.md#section-3-3)）：
...
### 4.1 把 `uintptr` 存进变量：最经典的 unsafe bug
...
1. **GC 回收**：`uintptr` 不是引用，不保活对象（[混合写屏障：为什么 Go 的停顿和堆大小无关](gc.md#section-2-2)）；
2. **栈移动**：goroutine 栈增长时会整体拷贝并按指针位图**修正所有指针**，但 `uintptr` 是整数，不会被修正。[copystack：栈上变量的地址会变](mem.md#section-3-2) 的实测里，一次 400 层递归就发生了 **4 次栈搬家**。
...
对称地，`unsafe.String(unsafe.SliceData(b), len(b))` 之后**不能再改 `b`**——否则"不可变"的字符串会在别人眼前变化（[`unsafe` 零拷贝转换（1.20+ 的正确写法）](string.md#section-2-2) 有实测）。
...
2. `Data` 是 `uintptr`——一旦你构造了一个 header **变量**，里面的 `Data` 就不再保活对象（[把 `uintptr` 存进变量：最经典的 unsafe bug](#section-3-1) 的坑）；
...
`unsafe.Pointer` 是**指针**：GC 认它、会保活指向的对象、栈移动时会被修正。`uintptr` 是**整数**：三者全不成立。所以 `uintptr` 只能作为表达式内部的临时算术中间值（见 [`unsafe.Pointer` 与 `uintptr` 的根本区别](#section-1-2)、[把 `uintptr` 存进变量：最经典的 unsafe bug](#section-3-1)）。

**2. 为什么不能把 `uintptr` 存进变量再转回 `Pointer`？**
两个独立原因：① GC 不认 `uintptr`，对象可能已被回收；② 栈增长时 runtime 会拷贝栈并修正所有指针，但不会修正整数。唯一正确形态是"转换 + 算术在同一个表达式里"（见 [把 `uintptr` 存进变量：最经典的 unsafe bug](#section-3-1)）。

**3. `unsafe.Sizeof` 是运行时调用吗？**
不是，它和 `Alignof`/`Offsetof` 都是**编译期常量**，可以用在 `const` 和数组长度里。`Sizeof(string)` 永远是 16（只算 header，不含数据）（见 [三个编译期常量函数](#section-1-1)）。

**4. Go 里为什么不能构造"尾后指针"（one-past-the-end）？**
GC 需要根据指针值定位它属于哪个对象（span 查找）。刚好指向对象末尾之后的地址会被误判成"下一个对象的开头"，造成错误保活或错误标记。C 里这是合法的，Go 里不是（见 [模式 ③：`Pointer` → `uintptr` 做算术 → **立刻**转回 `Pointer`](#section-2-2)）。

**5. 怎么零拷贝地把 `[]byte` 转成 `string`？有什么前提？**
1.20 起 `unsafe.String(unsafe.SliceData(b), len(b))`。前提：**转换后绝不修改 `b`**，且 `b` 的生命周期覆盖字符串的使用期。反方向（`string → []byte`）更危险：写字面量的内存会 SIGSEGV，且 recover 不了（见 [1.17/1.20 的新 API](#section-1-3)、[写只读内存 = SIGSEGV（不是 panic）](#section-3-3)、[`unsafe` 零拷贝转换（1.20+ 的正确写法）](string.md#section-2-2)）。

**6. `checkptr` 是什么？能完全依赖它吗？**
`-race`/`-d=checkptr` 插入的运行时检查，抓"未对齐的转换"和"越界的指针算术"。**不能完全依赖**：实测未对齐读 `[]byte` 的第 1 字节当 `int64` 没被拦住；而且结果必须真的被用到，否则可能被优化掉、检查也不插入（见 [`checkptr`：给 unsafe 加的运行时检查](#section-3-2)）。

**7. `reflect.SliceHeader` 为什么被废弃了？**
它的 `Data` 是 `uintptr`——一旦构造成变量，就不再保活对象。而且 Go 从未承诺 slice/string 的内存布局。1.20 起用 `unsafe.Slice`/`String`/`SliceData`/`StringData` 代替（见 [别自己拼 header](#section-3-4)）。

**8. `//go:linkname` 是什么？为什么不该用？**
绕过包边界直接绑定符号（需要 `import _ "unsafe"` 且函数只声明不实现）。runtime 内部符号无兼容承诺，1.23 起有 `-checklinkname` 拉黑了一批，历史上一大批库因此在 1.22+ 失效（见 [`//go:linkname`](#section-3-5)）。

**9. 用了 `unsafe` 一定更快吗？**
不一定。一旦对象地址流经 `unsafe.Pointer`，逃逸分析通常变保守，对象被搬到堆上——可能反而更慢。必须实测（见 [`unsafe` 与逃逸分析](#section-3-6)）。
...
C 不得长期保存 Go 指针（栈会移动、GC 不扫 C 内存）；传出去的 Go 指针所指内存里不能再含 Go 指针。长期共享用 `C.malloc` 或 `runtime/cgo.Handle`（见 [`unsafe` 与 cgo](#section-3-7)）。
