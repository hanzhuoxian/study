...
> 定位：函数值、闭包、defer 与调用机制。
> 前置知识：[Slice](slice.md)。
> 配套示例：[func/main.go](func/main.go)（`go run ./func`）；命令均在 notes 根目录执行。
...
- 命名返回值最重要的用途：**在 `defer` 中修改返回值**（详见 [defer 修改命名返回值](#section-3-1)）。
...
- 捕获可按值或按引用实现；是否堆分配由逃逸分析决定。逃出的闭包若需要共享可变捕获变量，该变量必须在外层函数返回后仍然存活。
- 多个闭包共享同一可变捕获变量时，必须保持修改互相可见；这种语义不等于所有闭包都需要堆分配。
...
逃逸分析决定对象能否留在栈上。返回闭包、返回指针或传给接口只是分析线索，并不意味着所有相关对象必然堆分配；内联与调用方的使用方式也会影响结果。

完整规则、编译诊断和实验统一见 [唯一的判定原则](mem.md#section-2-1)、[常见的逃逸原因](mem.md#section-2-2)。本篇只保留函数调用、捕获变量和 defer 的机制。
...
闭包是函数值 + 它所引用的外层作用域变量的组合。Go 闭包捕获的是**变量本身（引用语义）**，不是调用时的值快照；捕获变量是否堆分配取决于逃逸分析；共享同一可变变量的闭包观察同一份状态，任何一方修改都对其他方可见（见 [闭包（Closure）](#section-1-6)、[闭包的内部实现](#section-2-2)）。

**3. defer 的执行时机和顺序是什么？**
`defer` 在外层函数返回前（`return` 赋值之后）以 **LIFO** 顺序执行。多个 `defer` 像栈一样，最后注册的最先执行。`panic` 时也会执行（沿调用栈向上逐层执行 defer），直到被 `recover` 截获或进程崩溃（见 [defer](#section-1-7)）。

**4. `defer` 里修改返回值，调用方能看到吗？**
要看有没有用**命名返回值**。有命名返回值时，`return expr` 先赋值给命名返回值变量，再执行 defer，所以 defer 中对命名返回值的修改**会反映到调用方**。没有命名返回值时，`return` 创建的是临时拷贝，defer 无法修改（见 [命名返回值](#section-1-3)、[defer 修改命名返回值](#section-3-1)）。
...
**6. `defer` 的参数什么时候被求值？**
**立即求值**（在 `defer` 语句执行时），而不是延迟到调用时。若想让参数在执行时才求值，应把逻辑包在闭包里（`defer func() { use(x) }()`），这样 `x` 是通过引用捕获，会在执行时读取当前值（见 [defer 参数立即求值](#section-3-3)）。
...
**10. 如何让函数的局部变量不逃逸到堆上？**
不返回其地址、不被闭包捕获、不赋值给接口、不通过 `chan` 发送——让编译器的逃逸分析确认变量生命周期限于当前栈帧，就会分配在栈上。用 `go build -gcflags='-m'` 验证，减少堆分配能降低 GC 压力，提升性能（见 [逃逸分析](#section-2-4)）。
