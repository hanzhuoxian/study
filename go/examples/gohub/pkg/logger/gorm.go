...
// NewGormLogger 实例化
// DB,err := gorm.New(dbConfig, &gorm.Config{logger.NewGormLogger()})
...
// Info 实现 gorm logger.Interface 的 Info 方法
func (l GormLogger) Info(ctx context.Context, str string, args ...interface{}) {
	l.logger().Sugar().Debugf(str, args)
}

// Warn 实现 gorm logger.Interface 的 Warn 方法
func (l GormLogger) Warn(ctx context.Context, str string, args ...interface{}) {
	l.logger().Sugar().Warnf(str, args)
}

// Error 实现 gorm logger.Interface 的 Error 方法
...
func (l GormLogger) Trace(ctx context.Context, begin time.Time, fn func() (string, int64), err error) {
	...
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			...
			logFields = append(logFields, logFields...)
