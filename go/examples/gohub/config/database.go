	...
	config.Add("database", func() map[string]interface{} {
		return map[string]interface{}{
			...
			"mysql": map[string]interface{}{
				...
				"username": config.Env("DB_USERNAME", "gohub"),
				"password": config.Env("DB_PASSWORD", "gohub"),
				"charset":  config.Env("DB_CHARSET", "utf8mb4"),

				// 连接池配置
				"max_idle_connections": config.Env("DB_MAX_IDLE_CONNNECTIONS", 100),
				"max_open_connections": config.Env("DB_MAX_OPEN_CONNECTIONS", 25),
				"max_life_seconds":     config.Env("DB_MAX_FILE_SECONDS", 300),
			},
